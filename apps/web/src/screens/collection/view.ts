...
/** Les valeurs de `frameType` telles que l'API les écrit, en français. */
...
function groupTitle(item: CollectionItem): string {
  ...
  if (kind === "spell") return "Magie";
  if (kind === "trap") return "Piège";
  if (kind === "unresolved") return "À identifier";
...
const metaBits = (item: CollectionItem): SafeHtml => {
  ...
  if (card.linkValue !== null) {
    bits.push(html`<span class="meta-bit">Lien ${card.linkValue}</span>`);
  } else if (card.level !== null) {
    const isXyz = (card.type ?? "").includes("XYZ");
    bits.push(
      html`<span class="meta-bit">${levelBadge(isXyz ? "rank" : "level")}${isXyz ? "Rang" : "Niv."} ${card.level}</span>`,
    );
  }
  ...
  return html`<li class="item">
    <div class="item-row">
      <button type="button" class="item-main js-open" data-id="${item.id}">
        ...
        <div class="item-text">
          <strong>${item.card?.name ?? "Carte non identifiée"}</strong>
          ...
          <div class="item-meta">
            ...
            ${when(
              ...
              raw(`<span class="warn">en attente</span>`),
            )}
      ...
      <div class="item-actions">
        <button type="button" class="btn-qty js-fav${item.isFavorite ? " is-fav" : ""}"
                data-id="${item.id}" title="Favori"
                aria-label="Mettre en favori" aria-pressed="${String(item.isFavorite)}">★</button>
        ...
        <button type="button" class="btn-qty btn-qty-danger js-delta" data-id="${item.id}" data-d="-1"
                aria-label="Retirer un exemplaire">−</button>
      </div>
    </div>
  </li>`;
}
...
function tile(item: CollectionItem): SafeHtml {
  ...
  return html`<button type="button" class="tile js-open" data-id="${item.id}">
    <div class="tile-art">
      ...
      ${when(item.isFavorite, raw(`<span class="tile-star" title="Favori">★</span>`))}
    </div>
    <div class="tile-body">
      <strong class="tile-name">${item.card?.name ?? "Carte non identifiée"}</strong>
      <code>${item.setCode}</code>
    </div>
  </button>`;
}
...
export function contentHtml(state: ViewState): SafeHtml {
  if (state.loading) return raw(`<p class="web-loading muted">Chargement…</p>`);

  if (state.items.length === 0) {
    const filtered = state.total === 0 && hasActiveFilters(state);
    return html`<div class="empty-state">
      <p class="empty-title">${filtered ? "Aucun résultat" : "Aucune carte"}</p>
      <p class="muted">
        ${filtered
          ? "Modifiez les filtres."
          : "Saisissez le code en bas de la carte, puis appuyez sur +."}
      </p>
    </div>`;
  }
...
export function filterPanelHtml(state: ViewState, facets: Facets): SafeHtml {
  ...
  <div class="filter-panel" id="filter-panel" role="dialog" aria-modal="true"
       aria-label="Trier et filtrer" hidden>
    <div class="filter-panel-head">
      <h2>Trier &amp; filtrer</h2>
      <button type="button" class="icon-btn" id="filter-panel-close" aria-label="Fermer">✕</button>
    </div>

    <div class="filter-panel-body">
      <section class="filter-block">
        <h3 class="filter-block-title">Trier</h3>
        <div class="filter-row">
          ...
          <label class="menu-field">
            <span>Sens</span>
            <select id="sort-dir">
              ${[["desc", "Décroissant"], ["asc", "Croissant"]].map(
                ([value, label]) =>
        ...
        <div class="filter-row">
          ...
          <label class="menu-field">
            <span>Densité</span>
            <select id="density">
              ${[["comfort", "Confortable"], ["compact", "Compacte"]].map(
                ([value, label]) =>
        ...
        <label class="menu-check">
          ...
          Regrouper les monstres par type
        </label>
      </section>

      <section class="filter-block">
        <h3 class="filter-block-title">Catégorie</h3>
        <div class="chip-row">
      ...
      ${when(
        ...
        html`<section class="filter-block">
          <h3 class="filter-block-title">Attribut</h3>
          <div class="chip-row">
      ...
      ${when(
        ...
        html`<section class="filter-block">
          <div class="filter-block-head">
            <h3 class="filter-block-title">Propriété magie / piège</h3>
            <button type="button" class="help-q" id="btn-help-st"
              ...
              <span class="help-q-text">Aide</span>
            </button>
          </div>
          <div class="chip-row">
          ...
          <p class="filter-hint muted" id="help-st-hint" hidden>
            L'API range le type d'un monstre et la propriété d'une Magie ou d'un
            Piège dans le même champ. Un Piège « Normale » n'est donc pas un
            monstre Normal : ces deux listes sont séparées pour cette raison.
          </p>
        </section>`,
      )}
      ...
      ${when(
        ...
        html`<section class="filter-block">
          <h3 class="filter-block-title">Type de monstre</h3>
      ...
      <section class="filter-block">
        <h3 class="filter-block-title">Niveau</h3>
        <div class="chip-row">
      ...
      <section class="filter-block">
        <h3 class="filter-block-title">Rang Xyz</h3>
        <div class="chip-row">
      ...
      <section class="filter-block">
        <h3 class="filter-block-title">Valeur de Lien</h3>
        <div class="chip-row">
      ...
      ${when(
        ...
        html`<section class="filter-block">
          <h3 class="filter-block-title">Rareté</h3>
          <div class="chip-row">
      ...
      ${when(
        ...
        html`<section class="filter-block">
          <h3 class="filter-block-title">Langue de l'exemplaire</h3>
          <div class="chip-row">
    ...
    <div class="filter-panel-foot">
      <button type="button" class="btn" id="btn-clear-filters">Réinitialiser</button>
      <div class="filter-foot-actions">
        <button type="button" class="btn btn-primary" id="filter-panel-done">OK</button>
      </div>
    </div>
  </div>`;
}
...
export function inspectHtml(item: CollectionItem): SafeHtml {
  ...
  const rows: [string, string][] = card
    ? [
        ...
        [isSpellTrap ? "Propriété" : "Type monstre", translateRace(card.race, card.type) || "—"],
        ["Attribut", translateAttribute(card.attribute) || "—"],
        card.linkValue !== null
          ? ["Lien", String(card.linkValue)]
          : ["Niveau", card.level === null ? "—" : String(card.level)],
        [
          card.linkValue !== null ? "ATK" : "ATK / DEF",
          card.linkValue !== null
            ? String(card.atk ?? "—")
            : `${card.atk ?? "—"} / ${card.def ?? "—"}`,
        ],
  ...
  const owned: [string, string][] = [
    ...
    ["Édition", item.print.setName ?? "—"],
    ["Rareté", item.print.rarity || "—"],
    ["Langue", item.print.language.toUpperCase()],
    ["Exemplaires", String(item.quantity)],
  ];
  ...
  <div class="inspect-panel" id="inspect-panel" role="dialog" aria-modal="true">
    <button type="button" class="inspect-close icon-btn" id="inspect-close" aria-label="Fermer">✕</button>
        ...
        <div class="inspect-info-body">
          ...
          ${when(
            ...
            raw(
              ...
              `<p class="muted field-note">Traduction pas encore disponible — nom et texte en anglais.</p>`,
            ),
          )}
          ${when(
            Boolean(card?.desc),
            html`<div class="effect"><h3>Effet</h3><p>${card?.desc}</p></div>`,
          )}
          ${linkCompassHtml(card?.linkMarkers ?? null, card?.linkValue ?? null)}
          <h3>Votre exemplaire</h3>
          ${grid(owned)}
          <div class="inspect-notes">
            <label for="notes-input" class="muted">Note</label>
            <input type="text" id="notes-input" value="${item.notes ?? ""}"
                   maxlength="2000" placeholder="État, provenance, prix payé…" />
...
export function shellHtml(state: ViewState, facets: Facets): SafeHtml {
  return html`<main class="collection-page">
    <h1 class="page-heading">Ma collection</h1>
    <div class="pin-rail" id="pin-rail" data-pinned="${String(state.pinned)}">
      <div class="tools-bar">
        <label class="search">
          <span class="search-icon" aria-hidden="true">⌕</span>
          <input type="search" id="filter" placeholder="Rechercher une carte…"
                 aria-label="Rechercher dans la collection" value="${state.query}" />
        </label>
        <div class="view-toggle" role="group" aria-label="Mode d'affichage">
          <button type="button" class="icon-btn${state.view === "list" ? " is-active" : ""}"
                  data-view="list" title="Liste" aria-label="Vue liste"
          ...
          <button type="button" class="icon-btn${state.view === "gallery" ? " is-active" : ""}"
                  data-view="gallery" title="Galerie" aria-label="Vue galerie"
        ...
        <button type="button" class="icon-btn${state.pinned ? " is-active" : ""}" id="btn-pin"
                data-pin title="Épingler la barre" aria-label="Épingler la barre"
      ...
      <div class="add-bar">
        <div class="add-field">
          <input type="text" id="set-code" class="add-input" autocomplete="off"
                 ...
                 aria-label="Ajouter par set code"
                 placeholder="Code carte (ex. LTGY-FR008)" />
          <button type="button" class="add-chevron" id="btn-chevron" aria-expanded="false"
                  aria-label="Options (langue, rareté)" title="Options (langue, rareté…)">
            <span class="chevron" aria-hidden="true"></span>
          </button>
        </div>
        <button type="button" class="btn-add" id="btn-plus" title="Ajouter"
                aria-label="Ajouter la carte"><span aria-hidden="true">+</span></button>
        <button type="button" class="btn-scan" id="btn-scan" title="Scanner le set code"
      ...
      <div id="advanced-fields" class="add-advanced" hidden>
        <label class="add-select-wrap">
          <span class="add-select-label">Langue</span>
          <select id="opt-lang">
        ...
        <label class="add-select-wrap add-select-passcode">
          <span class="add-select-label">Passcode</span>
          <input type="text" id="opt-passcode" inputmode="numeric" autocomplete="off"
                 placeholder="optionnel" aria-label="Passcode de la carte" />
        </label>
      </div>

      <p class="meta-line">
        <span>${state.items.length}/${state.total} édition(s) · ${state.totalCopies} ex.</span>
<span class="warn">${state.pending > 0 ? `${state.pending} en attente d'identification` : ""}</span>
      </p>
    </div>
...
export function editionsHtml(prints: PrintRow[], ownedSetCodes: Set<string>): SafeHtml {
  ...
  return html`<div class="editions">
    <h3>
      Autres éditions
      <span class="muted">${prints.length}${possedees > 0 ? ` · ${possedees} possédée${possedees > 1 ? "s" : ""}` : ""}</span>
    </h3>
    <ul class="editions-list">
      ${prints.map((print) => {
        ...
        return html`<li class="edition-row${owned ? " is-owned" : ""}">
          ...
          ${owned
            ? raw(`<span class="edition-owned" title="Vous possédez cette édition">✓</span>`)
            : raw(`<span class="edition-missing muted" title="Pas dans votre collection">—</span>`)}
        </li>`;
      })}
    </ul>
  </div>`;
